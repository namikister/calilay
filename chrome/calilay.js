...
(async () => {
    const item = await chrome.storage.local.get("libraries");
    if (!item.libraries) throw new Error("Error: Could not get libraries");

    var libraries = item.libraries;

    var appkey = '0f316d2b698c28451ed3f5f5223df15b';

    chrome.runtime.sendMessage({method: "getSiteType", url: document.location.href}, (siteType) => {
        if (siteType === undefined ||
            (siteType === "AmazonKindle" &&
             !document.body.classList.contains("ebooks"))) {
            return;
        }
